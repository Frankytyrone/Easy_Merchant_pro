...
function runInstall(array $d): array
{
    ...
    $diagnoseToken  = bin2hex(random_bytes(24)); // 48 hex chars — secret for diagnose.php
    ...
    $eDiagnoseToken = var_export($diagnoseToken, true);
    $configContent = "<?php\n"
        ...
        . "// Secret token required to access diagnose.php — keep this private\n"
        . "define('DIAGNOSE_TOKEN', {$eDiagnoseToken});\n"
    ...
    // 5b. Write ebmpro/js/config.js with the diagnose token for the frontend
    $jsConfigDir  = dirname(__DIR__) . '/ebmpro/js';
    $jsConfigPath = $jsConfigDir . '/config.js';
    $jsConfigContent = "// Auto-generated by installer — do not edit manually\n"
        . "const DIAGNOSE_TOKEN = " . json_encode($diagnoseToken) . ";\n";
    if (is_dir($jsConfigDir)) {
        if (file_put_contents($jsConfigPath, $jsConfigContent) === false) {
            error_log('EBM installer: could not write ebmpro/js/config.js');
        }
    }

    // 6. Write install.lock (checked by install/.htaccess to block re-runs)
    if (file_put_contents(__DIR__ . '/installed.lock', date('Y-m-d H:i:s') . "\n") === false) {
        return ['ok' => false, 'msg' => 'Installation completed but could not write installed.lock. Please create it manually.'];
    }
    // Also write legacy install.lock for backwards compatibility with existing lock check at top of file
...
<form method="post" action="?step=4">
  <div class="form-cols">
    <div class="form-row">
      ...
      <input type="text" name="smtp_host" value="<?= val('smtp_host', 'smtp.shanemcgee.biz') ?>" placeholder="smtp.gmail.com">
    </div>
    <div class="form-row">
...
<div style="background:#fff5f5;border:2px solid #e53e3e;border-radius:8px;padding:16px 20px;margin:16px 0;color:#742a2a;">
  <strong style="font-size:1.05rem;">&#9888;&#65039; IMPORTANT: For security, please delete or rename the <code>/install/</code> folder from your server now that installation is complete.</strong>
</div>
